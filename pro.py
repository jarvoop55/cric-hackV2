...
async def hacke(c: Client, m: Message):
    ...
    try:
        ...
        # In the hacke function, replace the database query part
# Use cache for quick lookup
        if file_id not in player_cache:
            ...
            if file_data:
                ...
                logging.warning(f"Image ID {file_id} not found in {current_db_name} database.")
                return
...
async def import_json_to_mongodb(_, message: Message):
    ...
    try:
        ...
        # Download the file
        path = await bot.download_media(doc)
        ...
        result = await current_db.insert_many(data)
