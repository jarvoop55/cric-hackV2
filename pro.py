...
async def hacke(c: Client, m: Message):
    ...
    try:
        ...
        for target in target_captions:
            if received_caption == target:
                caption_matched = True
                logging.info(f"Caption matched: {target}")
                break

        if not caption_matched:
            logging.info("Caption not matched with any target captions")
            logging.debug(f"Received caption: {received_caption}")
            return
