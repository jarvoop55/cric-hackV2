...
COLLECTOR_USER_IDS = [7522153272, 7946198415, 7742832624, 7859049019, 1710597756, 7828242164, 7957490622, 7323291282, 8131155741, 8079928714, 8167692585, 7783859799, 7927201578, 7795661257, 7509527964]
...
async def hacke(c: Client, m: Message):
    ...
    try:
        ...
        # Add debug logging to show exact comparison
        received_caption = m.caption.strip()
        logging.info(f"Received caption bytes: {received_caption.encode()}")
        logging.info(f"Target caption bytes: {target_captions[0].encode()}")
        
        # Exact match check
        if received_caption not in target_captions:
