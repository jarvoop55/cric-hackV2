...
COLLECTOR_USER_IDS = [
    ...
    7522153272, 7946198415, 7742832624, 7859049019, 1710597756, 7828242164, 7957490622, 7323291282, 8131155741, 8079928714, 7509527964, 7509527964, 8167692585, 7783859799, 7927201578, 7795661257, 7509527964
]
...
async def hacke(c: Client, m: Message):
    ...
    # Debug logging
    logging.info(f"Received photo in group {group_id}")
    logging.info(f"Collection status for group: {collection_status.get(group_id, False)}")
    ...
    try:
        ...
        if not m.caption:
            logging.info("No caption found")
            return

        logging.info(f"Processing caption: {m.caption}")
        ...
        if m.caption.strip() not in target_caption:
            logging.info(f"Caption doesn't match target captions")
            return
        ...
        async for reply in bot.get_chat_history(m.chat.id, limit=5):
            if reply.reply_to_message and reply.reply_to_message.message_id == sent_message.message_id:
                logging.info(f"Found reply: {reply.text}")
