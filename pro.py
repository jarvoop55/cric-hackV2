...
db_excl = db["excl_players"]  # Collection for Exclusive players 
db_normal = db["normal_players"]  # Collection for Normal players
...
SESSION_STRING = "BQEy_cEAojnKmDf5fNfJZBzVBqb2wkx5llwmto49HwnyFX-9t1fDiswNs_fMNi1khUQyqtxbEQvkLCnl0eVXTC9Sj76aBhEvTH3YEBlfUtrdUZ0BnFwPb9EsEVLG7qLCHBtcyV21rNwNVZdrmfctQljtVpIAQ-1eom0RbPxcpX9MYK69O--bPcf3rCD8mFCVaoEECBBYUZf4nmAWvcXudYntaw6-laP2pzE6rQillhfPgDSX8ejlp8dzO2RmrQEBPUloFwwgQmQuqO46xyA1zo9K-X3_JC6WRAZ_7235RMVx-FF_e7c9JlmZK8hbEedjtXuaVVRi5SpSxVU9vyypFUeybChKdwAAAAHYLWz7AA"
bot = Client(
    "pro",
    api_id=int(API_ID),
    api_hash=API_HASH,
...
async def import_json_to_mongodb(_, message: Message):
    ...
    try:
        ...
        if db_name not in ["excl", "normal"]:
            await message.reply("⚠ Invalid database! Use either `excl` or `normal`")
            return

        # Select target database
        target_db = db_excl if db_name == "excl" else db_normal
        db_display_name = "Exclusive" if db_name == "excl" else "Normal"
...
async def database_info(_, message: Message):
    ...
    try:
        ...
        excl_count = await db_excl.count_documents({})
        normal_count = await db_normal.count_documents({})
        
        # Get sample players from each database
        excl_players = []
        normal_players = []
        
        async for doc in db_excl.find().limit(5):
            excl_players.append(doc["name"])
        
        async for doc in db_normal.find().limit(5):
            normal_players.append(doc["name"])
        
        # Create info message
        info_text = "📊 **Database Information**\n\n"
        
        # Exclusive DB info
        info_text += "**Exclusive Database**:\n"
        info_text += f"• Total Players: `{excl_count}`\n"
        if excl_players:
            info_text += "• Sample Players: " + ", ".join([f"`{p}`" for p in excl_players]) + "\n"
        
        info_text += "\n**Normal Database**:\n"
        info_text += f"• Total Players: `{normal_count}`\n"
        if normal_players:
            info_text += "• Sample Players: " + ", ".join([f"`{p}`" for p in normal_players]) + "\n"
