...
async def hacke(c: Client, m: Message):
    ...
    # Check if this client is from the active session
    active_session_string = SESSION_STRING_1 if CURRENT_SESSION == 1 else SESSION_STRING_2
    if c.session_string != active_session_string:
        return  # Silently ignore if not the active session
    ...
    try:
        ...
        # Send collect command using the active bot
        sent_message = await c.send_message(m.chat.id, f"/collect {player_name}")
        
        await asyncio.sleep(1)
        
        # Check for rare player response
        async for reply in c.get_chat_history(m.chat.id, limit=5):
...
async def switch_session(_, message: Message):
    ...
    try:
        ...
        await status_msg.edit_text(
            ...
            f"💡 Now only session {session_num} will collect"
        )
        logging.info(f"Switched active collection to session {session_num}")
        
    except Exception as e:
        logging.error(f"Error switching session: {e}")
        await message.reply(f"⚠ Error switching session: {str(e)}")

# Modify the main function to start both bots
async def main():
    """Initialize and run the bot"""
    global bot
    try:
        # Initialize first bot as default
        bot = create_bot(SESSION_STRING_1)
        await bot.start()
        logging.info("Bot initialized with Session 1")
        
        # Load players into cache
        await preload_players()
        
        # Start Flask health check in separate thread
        try:
        ...
        logging.info(f"Monitoring {len(TARGET_GROUP_IDS)} groups")
