...
# First modify the session tracking at the top
ACTIVE_SESSION = {
    "current": 1,  # Default to session 1
    "collecting": True  # Control collection state
}

# Initialize both bots at the start
bot1 = create_bot(SESSION_STRING_1)
bot2 = create_bot(SESSION_STRING_2)
bot = bot1  # Default bot
...
async def hacke(c: Client, m: Message):
    """Handle collection using only the active session."""
    group_id = m.chat.id
    
    if not collection_status.get(group_id, False):
        return

    # Check if this client matches the current active bot
    active_bot = ACTIVE_BOTS[CURRENT_SESSION]
    if not active_bot or c.session_string != active_bot.session_string:
        return
...
ACTIVE_BOTS = {
    1: None,  # Will store bot1 instance
    2: None   # Will store bot2 instance
}

@bot.on_message(filters.command("switchid") & filters.user(ADMIN_USER_IDS))
async def switch_session(_, message: Message):
    """Switch between different sessions by stopping one and starting the other."""
    global CURRENT_SESSION, ACTIVE_BOTS
    try:
        if len(message.command) != 2:
        ...
        # Stop the current active bot
        if ACTIVE_BOTS[CURRENT_SESSION]:
            await ACTIVE_BOTS[CURRENT_SESSION].stop()
            ACTIVE_BOTS[CURRENT_SESSION] = None
            logging.info(f"Stopped session {CURRENT_SESSION}")
        
        # Start the new bot
        new_session_string = SESSION_STRING_1 if session_num == 1 else SESSION_STRING_2
        new_bot = create_bot(new_session_string)
        await new_bot.start()
        ACTIVE_BOTS[session_num] = new_bot
        ...
        await status_msg.edit_text(
            ...
            f"**Active Session**: {session_num}\n"
            f"**Status**: Now collecting ONLY with session {session_num}"
        )
        logging.info(f"Switched to active session {session_num}")
...
# Modify the main function to initialize the first bot only
async def main():
    try:
        # Initialize only the first bot
        new_bot = create_bot(SESSION_STRING_1)
        await new_bot.start()
        ACTIVE_BOTS[1] = new_bot
        logging.info("Session 1 initialized successfully")
