...
async def hacke(c: Client, m: Message):
    ...
    current_session_string = SESSION_STRING_1 if CURRENT_SESSION == 1 else SESSION_STRING_2
    
    # Check if this client's session string matches the active session
    if c.session_string != current_session_string:
        # logging.info(f"Ignoring collection - not active session {CURRENT_SESSION}")
        return
