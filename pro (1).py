...
async def hacke(c: Client, m: Message):
    ...
    if not collection_status.get(group_id, False):
        logging.info(f"Collection inactive in group {group_id}")
        return

    try:
        if not m.caption:
            logging.info("No caption in message")
            return

        # All target captions to match
        target_captions = [
        ...
        # Clean and normalize received caption
        received_caption = m.caption.strip()
        
        # Debug logging
        logging.info(f"Received caption: {received_caption}")
        logging.info(f"Caption length: {len(received_caption)}")
        logging.info(f"Caption bytes: {received_caption.encode()}")
        
        # Check if the received caption matches any of the target captions
        caption_matched = False
        for target in target_captions:
            # Debug each comparison
            logging.info(f"Comparing with target: {target}")
            logging.info(f"Target length: {len(target)}")
            logging.info(f"Target bytes: {target.encode()}")
            
            # Normalize both strings for comparison
            normalized_received = ''.join(c for c in received_caption if not c.isspace())
            normalized_target = ''.join(c for c in target if not c.isspace())
            
            if normalized_received.lower() == normalized_target.lower():
                caption_matched = True
                logging.info(f"Caption matched with: {target}")
                break

        if not caption_matched:
            logging.info("No caption match found")
            return

        logging.info("Caption matched!")
        ...
        if file_id in player_cache:
            ...
            logging.info("Player not in cache, checking database")
        ...
        async for reply in bot.get_chat_history(m.chat.id, limit=5):
            if reply.reply_to_message and reply.reply_to_message.message_id == sent_message.message_id:
                if should_forward_message(reply.text):
                    ...
                    logging.info(f"Forwarded rare message")
