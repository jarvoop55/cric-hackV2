...
async def hacke(c: Client, m: Message):
    ...
    try:
        ...
        logging.info(f"Original caption: {received_caption}")

        def normalize_text(text):
            # Convert special Unicode characters to basic form
            replacements = {
                'ɢ': 'g',
                'ɴ': 'n',
                'ʟ': 'l',
        ...
        received_norm = normalize_text(received_caption)
        caption_matched = False

        for target in target_captions:
            target_norm = normalize_text(target)
            logging.info(f"Normalized comparison:\nReceived: {received_norm}\nTarget: {target_norm}")
            
            if received_norm == target_norm:
