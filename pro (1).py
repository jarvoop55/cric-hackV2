...
async def hacke(c: Client, m: Message):
    ...
    try:
        ...
        # Debug logging
        logging.info(f"Processing caption: {received_caption}")
        ...
        if received_caption in target_captions:
            ...
            # Try alternative comparison if exact match fails
            # Remove all special formatting and compare
            def clean_text(text):
                return ''.join(char for char in text.lower() if char.isalnum() or char.isspace())
            
            received_clean = clean_text(received_caption)
            caption_matched = False
            
            for target in target_captions:
                target_clean = clean_text(target)
                if received_clean == target_clean:
                    caption_matched = True
                    logging.info(f"Caption matched after cleaning: {target}")
                    break
